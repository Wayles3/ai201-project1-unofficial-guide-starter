...
for q in QS:
    res = answer(q)
    is_ref = "don't cover" in res["answer"].lower() or "doesn't cover" in res["answer"].lower() or "don't have" in res["answer"].lower()
    refused += is_ref
    print(f"{'REFUSED ' if is_ref else 'answered'}  {q}")
print(f"\n{refused}/{len(QS)} refused at generation despite all clearing the retrieval floor")
